...
def get_weather():
    try:
        ...
        print("##### response view:\n", response)
        print("##### response.status_code view:\n", response.status_code)
        print("##### response.json():\n", response.json())
        return jsonify(response.json()), 200
    except ValidationError as err:
        return jsonify({
            "ValidationError": str(err),
            "ValidationError message": str(err.message),
        }), response.status_code
    except BadResponseError as err:
        return jsonify({
            "BadResponseError": str(err),
            "BadResponseError message": str(err.message),
            "BadResponseError text": str(err.response_text),
        }), response.status_code